...
public:
	...
	{ 
#ifdef _DEBUG
		Msg("DEBUG: CPassThruMaterialSystem::UpdateConfig called. bForceUpdate=%d\n", bForceUpdate);
#endif
		if ( !m_pBaseMaterialsPassThru )
		{
#ifdef _DEBUG
			Warning("DEBUG: m_pBaseMaterialsPassThru is NULL! Cannot update config.\n");
#endif
			return false;
		}
		
#ifdef _DEBUG
		Msg("DEBUG: SKIPPING m_pBaseMaterialsPassThru->UpdateConfig to test fix\n");
#endif
		// request a new lightmap redownload, duct tape and elmer's glue fix for the crash
		if ( m_bRequestOneLightmapRedownload )
		{
			m_bRequestOneLightmapRedownload = false;
#ifdef _DEBUG
			Msg("DEBUG: One-time lightmap redownload requested for this level\n");
#endif
			return true;
		}
