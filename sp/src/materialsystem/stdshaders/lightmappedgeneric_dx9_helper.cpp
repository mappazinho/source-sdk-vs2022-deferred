...
<<<<<<< HEAD
	...
	if ( iDeferredRenderStage == DEFERRED_RENDER_STAGE_SHADOWPASS )
	{
		defParms_shadow parms_shadow;
		parms_shadow.bModel = false;
		parms_shadow.iAlbedo = info.m_nBaseTexture;
		parms_shadow.iAlphatestRef = info.m_nAlphaTestReference;
		parms_shadow.iAlbedo2 = info.m_nBaseTexture2;

		DrawPassShadowPass( parms_shadow, pShader, params, pShaderShadow, pShaderAPI,
			VERTEX_COMPRESSION_NONE, &pContextData->m_DeferredContext );
		return;
	}
